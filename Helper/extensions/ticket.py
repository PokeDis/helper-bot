...
class TicketCloseView(discord.ui.View):
    ...
    async def delete_ticket(
        ...
        await interaction.response.send_message(
            "Deleting the ticket in 5 seconds.", ephemeral=False
        )
...
class TicketCreateView(discord.ui.View):
    ...
    async def create_ticket(
        ...
        for text_channel in interaction.guild.text_channels:
            if interaction.user.name.lower() in text_channel.name:
                return await interaction.followup.send(
                    f"You already have an open ticket. Please head towards {text_channel.mention}.",
                    ephemeral=True,
                )
        overwrites = {
        ...
        await interaction.followup.send(
            f"Created ticket. Please head towards {channel.mention}.", ephemeral=True
        )
        ...
        view = InsideTicketView()
        embed = discord.Embed(title="Support Ticket", color=0x2F3136)
...
class Ticket(commands.Cog, description="Ticket related commands."):
    ...
    @commands.command(
        brief="Add a member to the ticket", help="Add a member to the ticket."
    )
    ...
    @commands.command(
        brief="Remove a member from the ticket", help="Remove a member from the ticket."
    )
    @commands.has_permissions(manage_channels=True)
    @commands.guild_only()
